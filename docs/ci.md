...
通过入口门禁后，GitHub-hosted 预检重新验证成功的 main proof、attestation、候选清单和摘要。三个场景 job 均固定使用 GitHub-hosted `windows-2025` 并在第一步断言 `RUNNER_ENVIRONMENT=github-hosted`。仓库已经公开 raw schema、来宾/控制器参考 harness 和独立 raw verifier，但尚未接入外部 VM adapter/JIT 服务，因此这些 job 仍按设计 fail closed，汇总任务也不会生成成功回执。未来真实 VM 只能由 GitHub-hosted job 使用 protected environment secret 调用仓库外、隔离、短生命周期的 adapter/JIT 服务；不得为本仓库注册持久化 runner。

PR 与 main 还无条件运行 GitHub-hosted Windows observer integration：它执行来宾 harness 中同一段 production C#，验证同 PID 多 HWND、轮询间瞬态窗口和 unhook fail-closed；exact-SHA artifact 在 main 上进入签名 validation proof。该快速集成只验证观察器本身，不替代真实安装旅程。
...
After the entry guard, a GitHub-hosted preflight revalidates the successful main proof, attestations, candidate manifest, and digests. All three scenario jobs are fixed to GitHub-hosted `windows-2025` and begin by asserting `RUNNER_ENVIRONMENT=github-hosted`. The repository now contains the raw schemas, guest/controller reference harnesses, and independent raw verifier, but it is not connected to an external VM adapter/JIT service; the jobs therefore still fail closed and the aggregate cannot issue a success receipt. A future real-VM path may only let a GitHub-hosted job use a protected-environment secret to call an external, isolated, short-lived adapter/JIT service; it must never register a persistent runner to this repository.

PR and `main` also run an unconditional GitHub-hosted Windows observer integration. It executes the same production C# embedded in the guest harness and verifies multiple HWNDs under one PID, a transient window between polling intervals, and unhook fail-closed behavior. Its exact-SHA artifact enters the signed main validation proof. This fast integration verifies the observer only; it does not replace a real installed journey.
