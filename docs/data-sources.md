...
| Backtest execution status | Tushare → BaoStock → AKShare |
...
Execution status is routed and cached independently from price bars. Tushare provides the strict `authoritative` grade by combining a complete exchange calendar with explicit historical suspension and daily price-limit datasets. BaoStock provides the first `basic_no_price_limits` fallback from its explicit calendar and `tradestatus` field. AKShare provides a second basic fallback for daily and weekly runs by independently reading the Sina trade-date history and unadjusted daily stock series. It is accepted only when every declared open day has exactly one positive-volume stock row and no stock row falls outside that calendar; any missing, duplicate, zero-volume, or extra day rejects the complete status snapshot. This deliberately supports ranges with fully observed trading while refusing to infer a suspension from a missing bar.

The basic grade enforces T+1, exact next-period opening time, and open-price availability, but deliberately does not claim or apply historical upper/lower price-limit evidence. BaoStock additionally carries explicit normal/suspended states; AKShare fails closed instead of representing a range containing a missing open-day row. Neither route infers tradability from zero volume, a missing bar, or approximate board/ST/IPO rules. Local TDX and Eastmoney remain unsupported for execution status.
...
Execution-status diagnostics exercise the Tushare calendar, suspension, limit, and raw-open permissions independently. BaoStock diagnostics verify SDK session initialization, calendar access, and explicit `tradestatus`/raw-open coverage while declaring that price-limit evidence is absent. AKShare declares its basic execution-status capability but validates the Sina calendar/daily-row equality only when a concrete update is requested. Missing permission, unknown suspension state, or incomplete evidence is actionable and fail-closed; it is never shown as tradable.
