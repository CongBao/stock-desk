...
仓库保留的 GitHub/SignPath updater 代码是历史且不可达的脚手架，不是 v1.2 Store/MSIX 方案。v1.2 的签名、更新和数据迁移必须在独立设计与验收中重新定义。
...
The retained GitHub/SignPath updater code is a historical, unreachable scaffold, not the v1.2 Store/MSIX design. Signing, updates, and data migration for v1.2 require a separate design and acceptance process.
