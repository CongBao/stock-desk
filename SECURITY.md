...
Pull requests are subject to dependency-diff review, locked Python and Web production-dependency audits, application boundary tests, and Bandit checks that fail on medium or high severity findings. Container builds generate an SPDX JSON software bill of materials (SBOM) and retain a JSON report containing every high or critical finding, including findings for which the distributor has not published a fix. A second scan fails the workflow when a high or critical finding has an available fix, so actionable upgrades cannot be deferred while unresolved upstream findings remain visible in the run artifacts. A missing vulnerability database, scanner failure, unavailable audit service, or missing report artifact fails the workflow; it is not treated as a passing result.
