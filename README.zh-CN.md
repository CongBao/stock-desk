...
Stock Desk `v0.4.0` 是一个本地优先的 A 股行情、公式与历史回测工作台。Stage 1 提供可配置行情源、持久化更新、来源追踪、股票池、计划任务，以及日线、周线、60 分钟交互图；Stage 2 新增受控的通达信兼容公式引擎与不可变版本；Stage 3 将已保存的 BUY/SELL 信号用于可复现的单股或股票池回测，并明确披露 A 股成交规则、成本、进度、报告和固定回放。

LLM 智能分析仍属于后续阶段；其导航入口只是规划预览，不代表能力已经完成，详见[路线图](ROADMAP.md)。
...
Stage 0 基础能力继续保留：`/market`、`/formulas`、`/backtests`、`/analysis`、`/tasks` 与 `/settings` 共用同一工作台外壳，`demo.double` 持久化任务仍可用于 worker 诊断。Stage 1 已完成行情数据，Stage 2 已完成公式，Stage 3 已完成历史策略回测；智能分析仍是预览。
...
Stage 3 包含行情工作区、技术指标/交易系统公式和可复现历史回测。不包含实时行情、动态选股器、画线工具、共享资金组合模拟、券商连接、实盘/自动交易或 LLM 分析。
...
先用 `pnpm exec playwright install chromium` 安装 Chromium，再运行 `make e2e-market`、`make e2e-formula` 与 `make e2e-backtest`，即可验证真实 Stage 1–3 浏览器流程。`make security` 需要网络访问：它通过 OSV 检查 Python 依赖，并通过 npm registry 检查 JavaScript 生产依赖；执行审计前还会确认清单与锁文件一致。Docker 运行时，`make release-check` 会执行全部浏览器、安全和隔离的容器 smoke 门禁，并自行启动、清理 Compose。项目采用 Apache-2.0 许可证。Stock Desk 是研究软件，不构成投资建议；请独立核验数据和决策。
