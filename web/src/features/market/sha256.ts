export const MAX_SHA256_INPUT_BYTES = 1024 * 1024;
...
function assertInputSize(bytes: Uint8Array): void {
  if (bytes.length > MAX_SHA256_INPUT_BYTES) {
    throw new RangeError('SHA-256 input exceeds the byte limit');
  }
}
...
export function sha256FallbackHex(bytes: Uint8Array): string {
  assertInputSize(bytes);
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const sigma0 =
        rotateRight(earlier15, 7) ^
        rotateRight(earlier15, 18) ^
        (earlier15 >>> 3);
      const sigma1 =
        rotateRight(earlier2, 17) ^
        rotateRight(earlier2, 19) ^
        (earlier2 >>> 10);
      words[index] =
        ((words[index - 16] ?? 0) +
          sigma0 +
...
export async function sha256Hex(bytes: Uint8Array): Promise<string> {
  ...
  if (subtle === undefined) return sha256FallbackHex(bytes);
